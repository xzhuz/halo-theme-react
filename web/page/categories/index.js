...
class Categories extends React.PureComponent {
	...
	async handlePageClick(data) {
		...
		const res = await http.getCategoryPostBySlug({ slug, ...data });
		if (res.status !== 200) {
			return;
		}
...
Categories.getInitialProps = async ctx => {
	...
	let posts = {};
	if (__isBrowser__) {
		posts = (await http.getCategoryPostBySlug({ slug, page: 0, size: 9 })).data;
	} else {
		posts = (await ctx.service.api.getCategoryPostBySlug({ slug, page: 0, size: 9 })).posts;
	}
